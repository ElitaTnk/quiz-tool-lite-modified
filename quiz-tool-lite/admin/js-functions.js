	...
	else
	{
		...
		switch (qType)
		{
			...
			case "blank":
				...
				var correctAnswerArrayDecoded = decodeURI(correctResponse).split('|');
        var correctAnswerArray = [];

        for(var i=0; i < correctAnswerArrayDecoded.length; i++) {
          var correctAnswerArrayComma = correctAnswerArrayDecoded[i].replace(new RegExp('%2C', 'g'), ',');
          correctAnswerArray.push(correctAnswerArrayComma);
          //console.log("kajsajsaisas" ,correctAnswerArrayComma );

        }


				// Get the correct feedback and put it into an array
				var correctFeedbackArrayDecoded = decodeURI(correctFeedbackArray).split('|');
        var correctFeedbackArray = [];

        for(var i=0; i < correctFeedbackArrayDecoded.length; i++) {
          var correctFeedbackArrayComma = correctFeedbackArrayDecoded[i].replace(new RegExp('%2C', 'g'), ',');
          correctFeedbackArray.push(correctFeedbackArrayComma);
          //console.log("kajsajsaisas" ,correctAnswerArrayComma );

        }

				// Get the incorrect feedback and put it into an array
				var incorrectFeedbackArrayDecoded = decodeURI(incorrectFeedbackArray).split('|');
        var incorrectFeedbackArray = [];

        for(var i=0; i < incorrectFeedbackArrayDecoded.length; i++) {
          var incorrectFeedbackArrayComma = incorrectFeedbackArrayDecoded[i].replace(new RegExp('%2C', 'g'), ',').replace(new RegExp('%24', 'g'), '$');
          incorrectFeedbackArray.push(incorrectFeedbackArrayComma);
          console.log("kajsajsaisas" ,incorrectFeedbackArrayComma );
          ...
          //remove whitespace
          for (var w=0; w < correctArray.length; w++) {
            correctArray[w]=correctArray[w].trim();
          }
					...
					if(!myResponse){
            myResponseText="None given";
          } else {
            myResponseText = myResponse;
          }
